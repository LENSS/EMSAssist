...
def build_vocab_tokenizer(model_uri, do_lower_case):
  ...
  vocab_file = os.path.join(model_uri, 'vocab', '30k-clean.vocab')
  spm_model_file = os.path.join(model_uri, 'vocab', '30k-clean.model')
  print(vocab_file)
...
def create_classifier_model(bert_config,
  ...
  """
  albert_config = AlbertConfig.from_json_file(bert_config)
  if initializer is None:
    initializer = tf.keras.initializers.TruncatedNormal(
        stddev=albert_config.initializer_range, seed=global_seed)
  ...
  albert_layer = AlbertModel(config=albert_config, float_type=tf.float32)
...
#  if not is_testing:
  albert_model.load_weights(hub_module_url)
...
#  pooled_output, _ = bert_model([input_word_ids, input_mask, input_type_ids])
  output = tf.keras.layers.Dropout(rate=albert_config.hidden_dropout_prob)(
      pooled_output)
...
def create_model(num_classes,
  ...
  bert_config = os.path.join(args.init_model, "config.json")
  init_h5 = os.path.join(args.init_model, "tf2_model.h5")
  bert_model, _ = create_classifier_model(
      bert_config,
      num_classes,
      max_seq_length = args.max_seq_len,
      hub_module_url=init_h5,
      hub_module_trainable=args.bert_trainable,
      is_testing=args.do_test,
      is_tf2=True)
...
#    saved_model_dir = os.path.join(model_dir, "{epoch:04d}")
    saved_model_dir = os.path.join(model_dir, "{epoch:04d}.h5")
...
def model_fit(bert_model, train_ds, validation_ds, cp_callback, train_epoch):
    ...
    print("finish training..., saving architecture")
...
#        print("testing the model: %s" % args.init_model)
    ...
    bert_model = create_model(num_classes = test_meta_data['num_classes'], args = args)
    bert_model.load_weights(best_model_ckpt)

#    bert_model = tf.keras.models.load_model(best_model_ckpt)
#    bert_model.compile(
#            optimizer='adam',
#            loss=tf.keras.losses.SparseCategoricalCrossentropy(),
#            metrics=[top1_metric_fn(), top3_metric_fn(), top5_metric_fn()])
...
#        model_test(bert_model, test_ds)
        ...
        best_bert_model = model_test(test_ds, test_meta_data, args)
