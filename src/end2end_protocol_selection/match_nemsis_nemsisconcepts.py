...
def vectorizeNEMSISInput(concept_dict, concept_list_f, pid_dict):
    #  concept_list_f -- the file contains the ~|~ separated concepts for each example
    concepts_list = util.readNEMSISFile(concept_list_f, discard_header = False)
    label_ids = []
    # |protocols| x |concepts|
...
#    nemsis_input_mml_concept_file = "nemsis_input_mml_concept.txt"
    ...
    get_nemsis_input_concept(codes2mmconcepts_dict, args.input_mm_file, refinfo)
    get_nemsis_input_concept(codes2mmlconcepts_dict, args.input_mml_file, refinfo)
...
#    print("fitted protocols nemsis id list: %s" % (len(nemsis_id_list)))
    ...
    if args.concept_set_source == "protocol":
...
#    print("protocol: average tamu metamaplite set vectorize tamu protocol encode rate: ", np.average(mmlc_pers))
    ...
    c_ivs, c_iers, c_ilabel_ids = vectorizeNEMSISInput(c_dict, args.input_mm_file, pid_dict)
    mmlc_ivs, mmlc_iers, mmlc_ilabel_ids = vectorizeNEMSISInput(mmlc_dict, args.input_mml_file, pid_dict)
...
#    vectorizeInputConcepts(args)
#    c_pvs, c_ivs, c_ilabel_ids = vectorizeInputConcepts(args)
    c_pvs, mmlc_pvs, c_ivs, mmlc_ivs, c_ilabel_ids, mmlc_ilabel_ids = vectorizeInputConcepts(args)
#
    print("\n=============== protocol_selection ====================")
    pids = range(c_pvs.shape[0])
    c_top1, c_top3, c_top5 = get_top_score(c_pvs, c_ivs, pids, c_ilabel_ids, args)
    print("(cosine) input metamap topk: ", c_top1, c_top3, c_top5)
    mmlc_top1, mmlc_top3, mmlc_top5 = get_top_score(mmlc_pvs, mmlc_ivs, pids, mmlc_ilabel_ids, args)
...
if __name__ == "__main__":
    ...
    # March 09, 2022
    # We have fixed the metamap_concept_set.txt and metamaplite_concept_set.txt
    # We have metamap concept set and metamaplite concept set with size 869 and 1575, respectively
    # vectorizeInputConcepts(args)
