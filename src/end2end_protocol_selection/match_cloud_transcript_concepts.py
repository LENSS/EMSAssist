...
def match_for_spk_dir(main_args, spk_path, spk_dir, result_list, thread_num):
...
#        c_top1, c_top3, c_top5, mmlc_top1, mmlc_top3, mmlc_top5 = protocol_selection(main_args, mm_concept_f_path, mml_concept_f_path, label_f_path)
        ...
        acc_lines.append(top_k_acc)

    result_list[thread_num] = acc_lines
...
def match_for_dir(main_args):
    ...
    # print( "result_list: %sx%sx%s" %(len(result_list), len(result_list[0]), len(result_list[0][0])) )
    averaged_result = np.average(result_list, axis = 0)
    # print(averaged_result)
    return averaged_result

def sota_concept_matching(main_args):

#     parser = argparse.ArgumentParser(description = "control the functions for MetaMapMatching")
#     parser.add_argument("--regen", action='store_true', default=False, help="indicate whether to regenerate files for concept matching")
#     parser.add_argument("--concept_set_source", action='store', type=str, help="both")
...
#     parser.add_argument("--metric", action='store', type=str, help="cosine")
...
#     main_args = parser.parse_args()
    ...
    return match_for_dir(main_args)
...
#    parser.add_argument("--input_mml_file", action='store', type=str, default="nemsis_input_mml_concept.txt", help="protocol, nemsis, both", required=True)
    ...
    print(main_args.metric)
    print(main_args.concept_set_source)    
